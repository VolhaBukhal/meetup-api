...
import { userExists, createUser, getAllUsers, matchPassword, generateTokens, saveToken} from '../helpers/auth.js'
...
export const login = async (req, res) => {
    try {
        ...
        const userIsExist = await userExists(email)
        if ( !userIsExist) {
            res.status(status.not_found).json({message: `User with ${email} hasn't been found`})  
        } else {
            const validPassword = await matchPassword(password, userIsExist.password)
            if (!validPassword) {
                res.status(status.not_found).json({message: `Invalid password, please, try again`}) 
            } else {
                const { accessToken, refreshToken } = await generateTokens( userIsExist.user_id, userIsExist.role)
                res.send( {
